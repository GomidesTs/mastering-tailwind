...
const fileItem = tv({
  slots: {
    ...
    icon: 'rounded-full border-4 border-violet-100 bg-violet-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500 p-2 text-violet-600',
    deleteButton: ''
  },
...
export const FileInput = ({ name, size, state }: FileItemProps) => {
        ...
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <div className="flex w-full items-center gap-3">
            <div className="h-2 flex-1 rounded-full bg-zinc-100 dark:bg-zinc-600">
              <div
                className="h-2 rounded-full bg-violet-600 dark:text-violet-400"
                style={{
                  width: state === 'complete' ? '100%' : '80%'
                }}
              />
            </div>
            <span className="dark:text-zinc-300text-sm font-medium text-zinc-700">
